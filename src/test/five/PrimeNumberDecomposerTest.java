...
public class PrimeNumberDecomposerTest {
  ...
  public void test() {
     ...
     * candidates
     */
    assertEquals("Factors should be " + arrayAsString(this.factors) + ", but is " + arrayAsString(factors), arrayAsString(this.factors),
        arrayAsString(factors));
    assertEquals("Factors products should be " + arrayAsString(this.factorProducts) + ", but is " + arrayAsString(factorProducts),
        arrayAsString(this.factorProducts), arrayAsString(factorProducts));
    assertEquals("getUniquePrimeFactorsWithCount(" + n + "): First dimension of returned array should be size 2, is " + uniqueFactors.length, 2,
        uniqueFactors.length);
    assertEquals("getUniquePrimeFactorsWithCount(" + n + "): Unique primes should be " + arrayAsString(this.uniqueFactors[0]) + ", but are "
        + arrayAsString(uniqueFactors[0]), arrayAsString(this.uniqueFactors[0]), arrayAsString(uniqueFactors[0]));
    assertEquals("getUniquePrimeFactorsWithCount(" + n + "): Unique primes count should be " + arrayAsString(this.uniqueFactors[1]) + ", but are "
        + arrayAsString(this.uniqueFactors[1]), arrayAsString(this.uniqueFactors[1]), arrayAsString(uniqueFactors[1]));
  }
}
